/* This is a sligtly adapted version of the Adaptive Radix Tree.
 ...
 public:
  ...
  uint64_t Build(const std::vector<std::pair<uint64_t, uint64_t>> &data) {
    ...
    for (uint64_t i = 0; i < data.size(); ++i) {
      ...
      const uint64_t data_key = data[i].first;
      const uint64_t data_value = data[i].second;
  ...
  std::optional<uint64_t> find(const uint64_t lookup_key) {
    uint8_t key[8];
    swapBytes(lookup_key, key);
    Node* leaf = lookup(tree_, key, 8, 0, 8);
    if (!isLeaf(leaf))
      return std::nullopt;
    return std::optional<uint64_t>(getLeafValue(leaf));
  }

  /* Should only work for scale_size *1*. Since we want to have a fair comparison of *dense* index structures, we do not
   * consider a more generic version. */
  std::optional<uint64_t> lower_bound(const uint64_t lookup_key) {
    uint8_t key[8];
    swapBytes(lookup_key, key);
    ...
    if (found) return std::optional(it.value);
    return std::nullopt;
  }
  ...
  //                const std::string& _data_filename) const { return unique; }
...
// This address is used to communicate that search failed
      ...
      if (next) {
        if (!isLeaf(next)) {
          switch (next->type) {
            ...
            case NodeType4:
            case NodeType16: iter.stack[iter.depth].pos = next->count - 1;
              break;
            case NodeType48:
            case NodeType256: iter.stack[iter.depth].pos = 255;
              break;
          }
        }
        iter.stack[iter.depth].node = next;
  ...
  void loadKey(uintptr_t tid, uint8_t key[]) {
    ...
    const uint64_t org_key = (*data_)[tid].first;
  ...
  unsigned prefixMismatch(Node* node,
                          uint8_t key[],
                          unsigned depth,
                          unsigned maxKeyLength) {
...
// Compare the the key with the prefix, return the number matching bytes
  ...
  bool lookupPrefixMin(Node* root,
                       uint8_t* key,
                       uint32_t keyLength,
                       Iterator& iterator,
                       unsigned maxKeyLength) {
  ...
  bool bound(Node* n,
    ...
    while (true) {
      ...
      unsigned
          mismatchPos = prefixMismatch(n, key, depth, keyLength, maxKeyLength);
  ...
  Node* lookupPessimistic(Node* node,
                          uint8_t key[],
                          unsigned keyLength,
                          unsigned depth,
                          unsigned maxKeyLength) {
...
//void eraseNode256(Node256* node, Node** nodeRef, uint8_t keyByte);
  ...
  void erase(Node* node,
    ...
    if (isLeaf(*child)
      ...
      switch (node->type) {
        case NodeType4: eraseNode4(static_cast<Node4*>(node), nodeRef, child);
          break;
        case NodeType16:
  ...
  Node* tree_ = NULL;
  const std::vector<std::pair<uint64_t, uint64_t>>* data_;
  uint64_t data_size_;
};
